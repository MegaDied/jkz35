...
<body>
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Register
                    <small>Join the community via this portal</small>
                </h1>
                <ol class="breadcrumb">
    ...
    if(isset($_POST['username']) && !empty($_POST['username']) AND isset($_POST['mail']) && !empty($_POST['mail']) AND isset($_POST['password']) && !empty($_POST['password']) AND isset($_POST['repassword']) && !empty($_POST['repassword'])){
		...
		if($repassword==$password) {
			...
			mysqli_query($connection, "INSERT INTO users (username, password, email, hash) VALUES(
				'". $username ."', 
				'". $password ."', 
				'". $mail ."', 
				'". $hash ."') ");
...
Your account has been successfully made. You can now login with the following username:
...
Warning! If you didn't request this mail, you can just delete it.
";
			$headers = 'From:noreply@jkz35.xyz' . "\r\n"; // Set from headers
			$send=mail($to, $subject, $message, $headers); // Send our email
			//E-mail done
			echo "User has been made! Please read your email";
...
?>
				<form method="POST" action="signup.php" name="signup">
						...
						<div class="form-group">
							<label for="EmailRegisterInput">E-mail:</label>
